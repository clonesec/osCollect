    ...
    if (itemCount) {
      ...
      for(i = series.length - 1; i > -1; --i){
        if(!series[i].label || series[i].hide) continue;
				// cls: bug?
        // label = legend.labelFormatter(series[i].label); // original
        // labelMaxWidth = Math.max(labelMaxWidth, this._text.measureText(label, style).width); // original
        // labelMaxWidth = Math.max(labelMaxWidth, this._text.measureText(series[i].label, style).width); // <-cls
				var label_total = this.data[i].data[0][1].toString(); // <-cls
        label = legend.labelFormatter(series[i].label+" = "+label_total.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")); // <-cls
      }
      ...
      if (!options.HtmlText && this.textEnabled && !legend.container) {
        ...
        for(i = 0; i < series.length; ++i){
          ...
          // label = legend.labelFormatter(s.label); // orginal
					var label_total = this.data[i].data[0][1].toString(); // <-cls
          label = legend.labelFormatter(series[i].label+" = "+label_total.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
